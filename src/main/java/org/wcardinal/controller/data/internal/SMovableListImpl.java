...
public class SMovableListImpl<V> extends SMovableList<V> implements SMovableListContainer<V>, ControllerDynamicInfoHandler {
	String name = null;
	...
	final Map<Object, SMovableListDataImpl<V>> originToData = new HashMap<>();
	...
	public void addOrigin( final Object origin ) {
		if( originToData.containsKey( origin ) != true ) {
			originToData.put( origin, new SMovableListDataImpl<V>( this, origin ) );
		}
	}
	...
	public void handle( final Object origin, final SetDynamicDataMap map ) {
		final SMovableListDataImpl<V> data = originToData.get( origin );
	...
	boolean onAdd( final int index, final V value ) {
		for( final SMovableListDataImpl<V> data: originToData.values() ) {
	...
	@Override
	public void onPatches( final Object except, final SPatchesPacked<List<V>, SMovableListPatch<V>> patches ) {
		for( final SMovableListDataImpl<V> data: originToData.values() ) {
			if( data == except ) continue;
	...
	public void move( final int oldIndex, final int newIndex ){
		try( final Unlocker unlocker = lock() ) {
			if( oldIndex < 0 || values.size() <= oldIndex || newIndex < 0 || values.size() <= newIndex ) {
